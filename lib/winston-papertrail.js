...
var Papertrail = (exports.Papertrail = function(options) {
  var self = this;
  winstonTransport.call(self, options);
  ...
  self.disableTls =
    typeof options.disableTls === "boolean" ? options.disableTls : false;
  ...
  self.maxBufferSize = options.maxBufferSize || 1 * 1024 * 1024;
  ...
  function onErrored(err) {
    ...
    setTimeout(function() {
      ...
      if (
        self.connectionDelay < self.maxDelayBetweenReconnection &&
  ...
  function onConnected() {
    ...
    self.emit(
      "connect",
      "Connected to Papertrail at " + self.host + ":" + self.port
    );
...
//
//
// Inherit from `winstonTransport` so you can take advantage
// of the base functionality and `.handleExceptions()`.
//
util.inherits(Papertrail, winstonTransport);
...
Papertrail.prototype.log = function(level, msg, meta, callback) {
  ...
  if (typeof output !== "string") {
    output = util.inspect(output, false, self.depth);
  }
  ...
  if (meta) {
    if (typeof meta !== "object") {
      ...
      if (this.inlineMeta) {
        output +=
          " " +
          util.inspect(meta, false, self.depth).replace(/[\n\t]\s*/gm, " ");
      } else {
        output += "\n" + util.inspect(meta, false, self.depth);
      }
    }
  }
 ...
 */
Papertrail.prototype.sendMessage = function(
  hostname,
  program,
  level,
  message,
